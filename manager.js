...
let sortAsc = true;
let searchTerm = ''; 
let columnFilters = {}; // NEW: Tracks individual column filters
let activeFilterId = null; // NEW: Remembers which filter box you are typing in
...
window.fetchTargets = async function(tableName) {
    ...
    // Reset ALL filters on module change
    searchTerm = '';
    columnFilters = {};
    activeFilterId = null;
    ...
    try {
       ...
       masterData = data.map(row => {
            ...
            return {
                ...
                derived_rep: row.rep_name || row.Rep_Name || row['Rep Name'] || 'N/A',
...
// NEW: Universal Data Filter Engine
function getFilteredData() {
    return masterData.filter(row => {
        ...
        // 2. Column-Specific Filters (AND logic)
        for (const key in columnFilters) {
            const filterText = columnFilters[key];
            if (filterText) {
                let cellValue = (key === 'target' && row.current_input !== undefined) ? row.current_input : row[key];
...
function renderTable() {
    ...
    // Header generation with injected filter inputs
    ...
    // NEW: Restore input focus so user can type seamlessly without losing cursor
    if (activeFilterId) {
